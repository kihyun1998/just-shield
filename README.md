...
처음이라면 [단계별 적용 가이드](docs/usage.md)를 순서대로 따라 하면 빈 저장소에서 CI 연동까지 한 번에 끝난다. 옵션·종료 코드·출력 형식의 정확한 정의는 [CLI 레퍼런스](docs/cli.md)에 있다. 아래는 빠른 요약이다:
...
"태그를 커밋 SHA로 박으면 매번 해시를 찾아 넣어야 하고, 한 번 박으면 업데이트가 끊긴다" — SHA 핀을 꺼리는 가장 흔한 이유다. 그 번거로움은 SHA 핀 자체가 아니라 **도구 부재의 증상**이다. 두 단계가 자동화한다:

- **최초 핀** — `just-shield fix`가 가변 참조를 커밋 SHA로 교체하고 버전 주석(`# v1.2.3`)까지 단다. 손으로 해시를 찾을 일이 없다.
- **이후 갱신** — GitHub Dependabot 의 `github-actions` 생태계가 핀된 SHA 와 그 버전 주석을 함께 올리는 PR 을 주기적으로 연다. 머지만 하면 핀이 **불변이면서 동시에 최신**으로 유지된다 — 해시를 추적할 일도, 업데이트를 놓칠 일도 없다 (major 버전 점프는 PR 리뷰에서 걸러진다).

즉 fix(최초) + Dependabot(갱신)이면 **태그를 쓰는 것과 같은 유지보수 비용으로 불변 참조의 안전성**을 얻는다. "SHA 로 박으면 업데이트가 끊긴다"는 통념과 정반대다. `.github/dependabot.yml` 한 블록이면 켜진다:
...
**섭취 검사 규칙 10개 구현 완료.** R9의 권고 DB(`data/advisories.txt`)와 R2의 유명 액션 목록(`data/popular-actions.txt`)은 바이너리에 동봉된다 — 형식·갱신 절차는 각 파일 머리말 참조. 갱신 = 새 릴리스이므로 데이터만 바꿔치기하는 공격면이 없다.
