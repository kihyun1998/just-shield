...
| R2 | — | 예정 ([이슈 트래커](https://github.com/kihyun1998/just-shield/issues) 참조) |

R9의 권고 DB는 바이너리에 동봉된다(`data/advisories.txt` — 형식·갱신 절차는 파일 머리말 참조). 갱신 = 새 릴리스이므로 DB만 바꿔치기하는 공격면이 없다.
